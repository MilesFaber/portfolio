		...
		<div id="list-profile" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="PEP" title="${message(code: 'profile.PEP.label', default: 'PEP')}" />
					
						<g:sortableColumn property="biography" title="${message(code: 'profile.biography.label', default: 'Biography')}" />
				...
				<g:each in="${profileInstanceList}" status="i" var="profileInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${profileInstance.id}">${fieldValue(bean: profileInstance, field: "PEP")}</g:link></td>
					
						<td>${fieldValue(bean: profileInstance, field: "biography")}</td>
